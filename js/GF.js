var version         = 201810281800;         // Version == 최종수정일 시간 분
var updateString    = "2018-10-28 Changelog"
                    + "\n- 군수 시간표(베타) 기능 추가"
                    ;
...
$('#btn-timetable').off().on('click', function (e) {
    ...
    val.start           = 420;              // 8시 기준
    if(sw_interval == true){
        val.sw_interval = true;        // 확인 주기 적용여부
    ...
    for(var i in selectedList){
        val.area[i]     = parseInt(selectedList[i]);
    }
    val.sleepStart      = 99;
